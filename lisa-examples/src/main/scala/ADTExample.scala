object ADTExample extends lisa.Main {
  ...
  val define(product, constructors(pair)) = (A, B) --> (A, B)

  // Sum type with constructors left :: A -> A + B and right :: B -> A + B
  val define(sum, constructors(left, right)) = (A, B) --> A | B
  ...
  val define(option, constructors(none, some)) = A --> () | A
  ...
  val consInj = Theorem((l :: list(A), x :: A) |- !(l === cons(A) * x * l)) { sp ?=>
