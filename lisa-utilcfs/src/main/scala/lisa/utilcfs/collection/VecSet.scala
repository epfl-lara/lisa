package lisa.utilcfs.collection
...
sealed class VecSet[A] private (protected val evec: Vector[A], protected val eset: Set[A]) extends Set[A] with SetOps[A, VecSet, VecSet[A]] with IterableFactoryDefaults[A, VecSet]:
  // invariants:
