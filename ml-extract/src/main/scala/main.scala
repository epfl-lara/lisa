...
def scproofstep2line(ps: SCProofStep, stepNum: Int, premises: Seq[String], indent: Int, varPrefix: String): String = {
  ...
  "  " * indent + (ps match
    ...
    case LeftSubstIff(bot, t1, equals, lambdaPhi) => s"$varDecl = have(${sequent2code(bot)}) by LeftSubstIff(${index2stepvar(t1)})"
    case RightSubstIff(bot, t1, equals, lambdaPhi) => s"$varDecl = have(${sequent2code(bot)}) by RightSubstIff(${index2stepvar(t1)})"
    case InstSchema(bot, t1, mCon, mPred, mTerm) => s"$varDecl = have(${sequent2code(bot)}) by InstSchema(${index2stepvar(t1)})"
    ...
    // if sp.sp.length == 1 then scproofstep2line(sp.sp.steps(0), stepNum, sp.premises.map(index2stepvar), indent, varPrefix)
    // else s"$varDecl = have(${sequent2code(sp.bot)}) subproof {\n" + scproof2code(sp.sp, sp.premises.map(index2stepvar), indent + 1, s"$varPrefix$stepNum") + "\n" + "  " * indent + "}"
    case Sorry(bot) => s"$varDecl = have(${sequent2code(bot)}) by Sorry"
  )
}
...
object MLExtract extends lisa.Main {
  ...
  val thm21 = Theorem((∀(x, Q(x)) /\ ∀(x, R(x))) <=> ∀(x, Q(x) /\ R(x))) {
    ...
    val s2 = have(() ⊢ ((∀(x, Q(x)) ∧ ∀(x, R(x))) <=> ∀(x, (Q(x) ∧ R(x))))) subproof {
      ...
      val s22 = have((((∀(x, R(x)) ∧ ∀(x, Q(x))) ==> ∀(x, (Q(x) ∧ R(x)))), (∀(x, (Q(x) ∧ R(x))) ==> (∀(x, Q(x)) ∧ ∀(x, R(x))))) ⊢ ((∀(x, Q(x)) ∧ ∀(x, R(x))) <=> ∀(x, (Q(x) ∧ R(x))))) subproof {
        ...
        val s221 =
          have((((∀(x, R(x)) ∧ ∀(x, Q(x))) ==> ∀(x, (Q(x) ∧ R(x)))), (∀(x, (Q(x) ∧ R(x))) ==> (∀(x, Q(x)) ∧ ∀(x, R(x))))) ⊢ ((∀(x, Q(x)) ∧ ∀(x, R(x))) <=> ∀(x, (Q(x) ∧ R(x))))) by Restate.from(s220)
      }
  ...
  val thm31 = Theorem(∀(x, Q(x) ==> Q(f(x))) |- Q(x) ==> Q(f(f(x)))) {
    ...
    val s2 = have(∀(x_1, (Q(x_1) ==> Q(f(x_1)))) ⊢ (Q(f(x)) ==> Q(f(f(x))))) subproof {
      val s20 = have(∀(x_1, (Q(x_1) ==> Q(f(x_1)))) ⊢ (Q(f(x)) ==> Q(f(f(x))))) by InstSchema(s1)
    }
    val s3 = have(∀(x, (Q(x) ==> Q(f(x)))) ⊢ (Q(x) ==> Q(f(f(x))))) subproof {
      ...
      val s32 = have((∀(x, (Q(x) ==> Q(f(x)))), (∀(x, (Q(x) ==> Q(f(x)))) ==> (Q(x) ==> Q(f(x)))), (∀(x_1, (Q(x_1) ==> Q(f(x_1)))) ==> (Q(f(x)) ==> Q(f(f(x)))))) ⊢ (Q(x) ==> Q(f(f(x))))) subproof {
        val s320 =
          have(Q(f(x)) ⊢ ¬(∧(¬(∧(∀(x_1, ¬((Q(x_1) ∧ ¬(Q(f(x_1)))))), ⊤, ¬(Q(f(f(x)))))), ¬(∧(∀(x, ¬((Q(x) ∧ ¬(Q(f(x)))))), Q(x), ¬(⊤))), ∀(x, ¬((Q(x) ∧ ¬(Q(f(x)))))), Q(x), ¬(Q(f(f(x))))))) by Restate
        val s321 = have(
          Q(f(x)) ⊢ ¬(
            ∧(
              ¬(∧(∀(x_1, ¬((Q(x_1) ∧ ¬(Q(f(x_1)))))), Q(f(x)), ¬(Q(f(f(x)))))),
              ¬(∧(∀(x, ¬((Q(x) ∧ ¬(Q(f(x)))))), Q(x), ¬(Q(f(x))))),
              ∀(x, ¬((Q(x) ∧ ¬(Q(f(x)))))),
              Q(x),
              ¬(Q(f(f(x))))
            )
          )
        ) by RightSubstIff(s320)
        val s322 = have(
        ...
        ) by Restate.from(s325)
        val s327 =
          have((∀(x, (Q(x) ==> Q(f(x)))), (∀(x, (Q(x) ==> Q(f(x)))) ==> (Q(x) ==> Q(f(x)))), (∀(x_1, (Q(x_1) ==> Q(f(x_1)))) ==> (Q(f(x)) ==> Q(f(f(x)))))) ⊢ (Q(x) ==> Q(f(f(x))))) by Restate.from(
            s326
          )
      }
