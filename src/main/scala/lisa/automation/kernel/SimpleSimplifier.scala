...
object SimpleSimplifier {
        ...
        override def asSCProof(premises: Seq[Int], currentProof: lisa.utils.Library#Proof):ProofStepJudgement = {
            ...
            phi match {
                case PredicateFormula(label, args) if label == equality =>
                    ...
                    if (isolatedLeft.forall(_._2.isEmpty) && isolatedRight.forall(_._2.isEmpty)) return ProofStepJudgement.InvalidProofStep(this.asProofStep(premises), s"There is no instance of ${Printer.prettyTerm(left)} to replace.")
                ...
                case ConnectorFormula(label, args) if label == Iff =>
                    ...
                    if (isolatedLeft.forall(_._2.isEmpty) && isolatedRight.forall(_._2.isEmpty)) return ProofStepJudgement.InvalidProofStep(this.asProofStep(premises), s"There is no instance of ${Printer.prettyFormula(left)} to replace.")
    ...
    def simplifySequent(seq:Sequent): SCProof = {
        val (ruleseq, rulesiff, rem): (List[PredicateFormula], List[ConnectorFormula], List[Formula]) = seq.left.foldLeft((List[PredicateFormula](), List[ConnectorFormula](), List[Formula]()))((prev, f) => {
            f match {
