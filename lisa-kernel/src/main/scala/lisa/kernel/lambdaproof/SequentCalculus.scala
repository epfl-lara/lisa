...
object SequentCalculus {
   ...
   */
  def sequentToFormula(s: Sequent): Expression = Implies(And(s.left), Or(s.right))
   ...
   * </pre>
   */
  case class LeftExistsOne(bot: Sequent, t1: Int, phi: Expression, x: Variable) extends SCProofStep { val premises = Seq(t1) }
   ...
   * </pre>
   */
  case class RightExistsOne(bot: Sequent, t1: Int, phi: Expression, x: Variable) extends SCProofStep { val premises = Seq(t1) }
