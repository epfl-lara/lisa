...
class RunningTheory {
   ...
   */
  sealed case class PredicateDefinition private[RunningTheory] (label: ConstantPredKerLabel, expression: LambdaTermFormula) extends Definition
  ...
  private[proof] val predDefinitions: mMap[ConstantPredKerLabel, Option[PredicateDefinition]] = mMap(equality -> None, top -> None, bot -> None)
   ...
   */
  def makePredicateDefinition(label: ConstantPredKerLabel, expression: LambdaTermFormula): RunningTheoryJudgement[this.PredicateDefinition] = {
  ...
  def sequentFromJustification(j: Justification): Sequent = j match {
    ...
    case PredicateDefinition(label, LambdaTermFormula(vars, body)) =>
      val inner = ConnectorFormula(Iff, Seq(PredKerFormula(label, vars.map(VariableTerm.apply)), body))
    ...
    case FunctionDefinition(label, out, LambdaTermFormula(vars, body), _) =>
      val inner = BinderFormula(
        ...
        ConnectorFormula(
          Iff,
          Seq(
            PredKerFormula(equality, Seq(Term(label, vars.map(VariableTerm.apply)), VariableTerm(out))),
            body
          )
        )
      )
  ...
  def addSymbol(s: ConstantLabel): Unit = {
    if (isAvailable(s)) {
      ...
      s match {
        ...
        case c: ConstantPredKerLabel => predDefinitions.update(c, None)
      }
    } else {}
  }
  ...
  def belongsToTheory(phi: Formula): Boolean = phi match {
    case PredKerFormula(label, args) =>
      label match {
        case l: ConstantPredKerLabel => isSymbol(l) && args.forall(belongsToTheory)
  ...
  def isSymbol(label: ConstantLabel): Boolean = label match {
    ...
    case c: ConstantPredKerLabel => predDefinitions.contains(c)
  }
   ...
   */
  def getDefinition(label: ConstantPredKerLabel): Option[PredicateDefinition] = predDefinitions.get(label).flatten
  ...
  def getDefinition(name: Identifier): Option[Definition] = knownSymbols.get(name).flatMap {
    case f: ConstantPredKerLabel => getDefinition(f)
