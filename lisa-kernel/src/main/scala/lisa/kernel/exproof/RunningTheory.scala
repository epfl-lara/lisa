package lisa.kernel.exproof

import lisa.kernel.exfol.FOL._
import lisa.kernel.exproof.RunningTheoryJudgement._
import lisa.kernel.exproof.SequentCalculus._
...
class RunningTheory {
  ...
  private[exproof] val theoryAxioms: mMap[String, Axiom] = mMap.empty
  private[exproof] val theorems: mMap[String, Theorem] = mMap.empty

  private[exproof] val funDefinitions: mMap[ConstantFunctionLabel, Option[FunctionDefinition]] = mMap.empty
  private[exproof] val predDefinitions: mMap[ConstantAtomicLabel, Option[PredicateDefinition]] = mMap(equality -> None, top -> None, bot -> None)

  private[exproof] val knownSymbols: mMap[Identifier, ConstantLabel] = mMap(equality.id -> equality)
  ...
  def sequentFromJustification(j: Justification): Sequent = j match {
    ...
    case PredicateDefinition(label, LambdaTermFormula(vars, body)) =>
      val inner = ConnectorFormula(Iff, Seq(AtomicFormula(label, vars.map(VariableTerm.apply)), body))
    ...
    case FunctionDefinition(label, out, LambdaTermFormula(vars, body), _) =>
      val inner = BinderFormula(
        ...
        ConnectorFormula(
          Iff,
          Seq(
            AtomicFormula(equality, Seq(Term(label, vars.map(VariableTerm.apply)), VariableTerm(out))),
            body
          )
        )
      )
