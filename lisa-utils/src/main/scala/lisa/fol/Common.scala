...
trait Common {
   ...
   * It can be either a [[SchematicPredicateLabel]] or a [[ConstantPredKerLabel]].
   */
  ...
  case class VariableFormula(id: Identifier) extends SchematicAtomLabel with AtomicFormula with Absolute with SchematicLabel[Formula] {
    ...
    val underlying = K.PredKerFormula(underlyingLabel, Seq())
  ...
  case class ConstantFormula(id: Identifier) extends ConstantConstOrPredLabel with AtomicFormula with Absolute with ConstantLabel[Formula] {
    ...
    val args: Seq[Nothing] = Seq()
    val underlyingLabel: K.ConstantPredKerLabel = K.ConstantPredKerLabel(id, 0)
    val underlying = K.PredKerFormula(underlyingLabel, Seq())
    override def applyUnsafe(args: Term ** 0): Formula = this
  ...
  /**
   * A constant predicate label corresponding to [[K.ConstantPredKerLabel]].
   */
  case class ConstantPredKerLabel[N <: Arity](id: Identifier, arity: N) extends ConstantConstOrPredLabel with ConstantLabel[Term ** N |-> Formula] with ((Term ** N) |-> Formula) {
    val underlyingLabel: K.ConstantPredKerLabel = K.ConstantPredKerLabel(id, arity)
    ...
    def freeSchematicLabels: Set[SchematicLabel[?]] = Set.empty
    def allSchematicLabels: Set[SchematicLabel[?]] = Set.empty
    def rename(newid: Identifier): ConstantPredKerLabel[N] = ConstantPredKerLabel(newid, arity)
    def freshRename(taken: Iterable[Identifier]): ConstantPredKerLabel[N] = rename(K.freshId(taken, id))
    override def toString(): String = id
    def mkString(args: Seq[Term]): String = if (infix) (args(0).toString() + " " + toString() + " " + args(1).toString()) else toString() + "(" + args.mkString(", ") + ")"
    override def mkStringSeparated(args: Seq[Term]): String = if (infix) "(" + mkString(args) + ")" else mkString(args)
  }
  object ConstantPredKerLabel {
    def infix[N <: Arity](id: Identifier, arity: N): ConstantPredKerLabel[N] =
      val x = new ConstantPredKerLabel[N](id, arity)
      x.infix = true
      x
  }
  ...
  case class PredicateFormula(p: AtomLabel, args: Seq[Term]) extends AtomicFormula with Absolute {
    ...
    override val underlying = K.PredKerFormula(p.underlyingLabel, args.map(_.underlying))
    def substituteUnsafe(map: Map[SchematicLabel[_], LisaObject[_]]): Formula =
