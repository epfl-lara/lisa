...
object Serialization {
  ...
  def formulaLabelToString(label: FormulaLabel): String =
    label match
      case l: ConstantPredKerLabel => "cpl_" + l.id.name + "_" + l.id.no + "_" + l.arity
    ...
    def labelFromInputStream(dis: DataInputStream): Label = {
      ...
      labelType match
        ...
        case 3 =>
          ...
          val no = dis.readInt()
          val arity = dis.readInt()
          ConstantPredKerLabel(Identifier(name, no), arity)
        case 4 =>
          val name = dis.readUTF()
          val no = dis.readInt()
          val arity = dis.readInt()
    ...
    proofsFromDataStream(treesDIS, proofDIS).map { (name, proof, justifications) =>
      val justs = justifications.map { j =>
        ...
        j(0) match
          ...
          case 'p' =>
            name.split("_") match
              case Array(name, no, arity) => theory.getDefinition(ConstantPredKerLabel(Identifier(name, no.toInt), arity.toInt)).get
      }
      if debug then
