...
trait WithTheorems {
  library: Library =>
  type Test=ProofEnvironment


  abstract class ProofEnvironment(val goal:Sequent){
    val proofEnvironment: ProofEnvironment = this


    val baseProof: BaseProof = new BaseProof(goal)

    var activeProof: Proof = baseProof
    ...
    sealed class InnerProof[J<:Proof](val parent:J, val goal:Sequent) extends Proof(parent.getAssumptions) {
      ...
      override def sequentOfOutsideFact(of: parent.Fact): Sequent = of match {
        ...
        case ds:ProofEnvironment#Proof#DoubleStep => ds.bot
