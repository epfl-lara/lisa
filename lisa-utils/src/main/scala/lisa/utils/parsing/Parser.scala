...
class Parser(
  ...
  def printFormula(f: Formula): String = SequentParser
    ...
    .getOrElse({
      f match {
        case PredKerFormula(_, args) => args.foreach(printTerm)
  ...
  case class Termula(label: RangedLabel, args: Seq[Termula], range: (Int, Int)) {
    def toTerm: Term = label.folLabel match {
      ...
      case t: ConstantPredKerLabel => Term(ConstantFunctionLabel(t.id, t.arity), args.map(_.toTerm))
    ...
    def toFormula: Formula = label.folLabel match {
      case p: AtomLabel => PredKerFormula(p, args.map(_.toTerm))
  ...
  extension (f: Formula) {
    ...
    def toTermula: Termula = {
      ...
      f match {
        case PredKerFormula(label, args) => Termula(label, args.map(_.toTermula), UNKNOWN_RANGE)
  ...
  extension (t: Term) {
    def toTermula: Termula = {
      ...
      val newLabel = t.label match {
        case ConstantFunctionLabel(id, arity) => ConstantPredKerLabel(id, arity)
  ...
  private[Parser] object SequentParser extends Parsers with ParsingUtils {
    ...
    def reconstructPrefixApplication(t: Termula): Token ~ Option[RangedTermulaSeq] = {
      ...
      t.label.folLabel match {
        ...
        case ConstantPredKerLabel(id, arity) =>
    ...
    val prefixApplication: Syntax[Termula] = ((elem(IdKind) | elem(OrKind) | elem(AndKind)) ~ opt(args)).map(
      { case p ~ optArgs =>
        ...
        val l = p match {
          ...
          case ConstantToken(id, _) => ConstantPredKerLabel(synonymToCanonical.getInternalName(id), args.size)
    ...
    val infixFunctionLabels: List[PrecedenceLevel[RangedLabel]] = infixFunctions.map({ case (l, assoc) =>
      elem(InfixKind(l)).map[RangedLabel](
        {
          case InfixToken(`l`, range) => RangedLabel(ConstantPredKerLabel(synonymToCanonical.getInternalName(l), INFIX_ARITY), range)
          case _ => throw UnreachableException
        },
        {
          case RangedLabel(ConstantPredKerLabel(id, INFIX_ARITY), range) => Seq(InfixToken(synonymToCanonical.getPrintName(id), range))
          case _ => throw UnreachableException
        }
      ) has assoc
    })
    ...
    val connectorTermula: Syntax[Termula] = infixOperators[RangedLabel, Termula](subtermula)(
      ...
      {
        case Termula(pred @ RangedLabel(ConstantPredKerLabel(_, INFIX_ARITY), _), Seq(l, r), _) => (l, pred, r)
