...
object UnificationUtils2 {
  ...
  private def matchFormulaRecursive(using context: RewriteContext)(reference: Formula, template: Formula, formulaSubstitution: FormulaSubstitution, termSubstitution: TermSubstitution): Option[(FormulaSubstitution, TermSubstitution)] = {
    ...
    def getContextTerm(first: Term, second: Term, freeTermRules: Seq[(Term, Term)], confinedTermRules: Seq[(Term, Term)] = Seq.empty, takenTermVariables: Set[VariableLabel] = Set.empty): Option[TermRewriteLambda] = {
      val context = RewriteContext(
    ...
    private def getContextRecursive(using context: RewriteContext)(first: Term, second: Term): Option[TermRewriteLambda] = {
      ...
      val validSubstitution = 
        context.confinedTermRules.collectFirst {
        case (l, r) => 
        ...
        freeContext.freeTermRules.collectFirst {
        case (l, r) => 
      ...
      if (first == second)
        ...
        if (innerSubstitutions.exists(_.isEmpty))
          None
        else {
          val retrieved = innerSubstitutions.map(_.get)
          val body = Term(first.label, retrieved.map(_.body))
          val lambda = 
            retrieved.foldLeft(TermRewriteLambda(body = body)) {
              case (currentLambda, nextLambda) =>
                TermRewriteLambda(
  ...
  private def getContextRecursive(using context: RewriteContext)(first: Formula, second: Formula): Option[FormulaRewriteLambda] = {
    // check if there exists a substitution
      val validSubstitution = 
        context.confinedFormulaRules.collectFirst {
        case (l, r) => 
          val subst = canRewrite(using context)(first, second, (l, r))
          subst match {
            case Some(s) => ((l, r), s)
        ...
        val freeContext = context.copy(takenTermVars = Set.empty)
        freeContext.freeFormulaRules.collectFirst {
        case (l, r) => 
          val subst = canRewrite(using freeContext)(first, second, (l, r))
          subst match {
            case Some(s) => ((l, r), s)
      ...
      if (first == second)
        ...
        (first, second) match {
          case (BinderFormula(labelF, boundF, innerF), BinderFormula(labelS, boundS, innerS)) => {
            ...
            innerSubst.map(s =>
              s.copy(body = BinderFormula(labelF, boundF, s.body))
            )
          }
          ...
          case (ConnectorFormula(labelF, argsF), ConnectorFormula(labelS, argsS)) => 
            if (argsF.length != argsS.length)
              ...
              if (innerSubstitutions.exists(_.isEmpty))
                ...
                val lambda = 
                  retrieved.foldLeft(FormulaRewriteLambda(body = body)) {
                    case (currentLambda, nextLambda) =>
