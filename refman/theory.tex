...
ZF Set Theory stands for Zermelo-Fraenkel Set Theory. It contains a set of initial predicate symbols and function symbols, as shown in \Cref{fig:symbolszf}. It also contains the 7 axioms of Zermelo (\Cref{fig:axiomsz}), which are technically sufficient to formalize a large portion of mathematics, plus the axiom of replacement of Fraenkel (\Cref{fig:axiomszf}), which is needed to formalize more complex mathematical theories.
...
This also naturally corresponds to \textit{comprehensions} over collections in programming languages, as in \Cref{tab:comprehensionsProgramming}.
\begin{table}[h]
  \begin{tabular}{l|l}
    ...
    Those are typically syntactic sugar for a more verbose expression. For example in scala, \lstinline|(0 to 9).filter(x => x % 2 == 0).map(x => -x)|. However this kind of expressions is not possible in first-order logic: We can't built in any way a term that contains formulas as subexpressions, as in \lstinline|filter|. So if we want to use such constructions, we need to simulate it as we did for local definitions in \Cref{sec:localDefinitions}.
      ...
      Note that the replacement axiom \Cref{axzf:replacement} is conditional of the schematic symbol $\psi$ being a functional relation. It is more convenient to move this condition inside the axiom, to obtain a non-conditional equivalence. This is the approach adopted in Isabelle/ZF \cite{noelExperimentingIsabelleZF1993}. We instead can prove and use
      ...
      Moreover, we most often want to map a set by a known function. In those case, Lisa provides refined versions \lstinline|t.filter|, \lstinline|t.map| and \lstinline|t.collect|, which are detailed in \Cref{tab:comprehensions}. In particular, these versions already prove the functionality requirement of replacement.
      \begin{table}[h]
        \begin{tabular}{l|l}
