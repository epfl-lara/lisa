...
\begin{tabularx}{0.9\columnwidth}{|l|X|}
  \lstinline|draft()| & Theorems outside of the curent file are assumed and not checked for correctness. 
  \\[4ex]
  \lstinline|withCache()| & Kernel proofs will be stored in binary files when theorems are constructed. On future runs with the option enabled, theorems will be constructed from the stored low level proofs. This skip running tactics.\\
