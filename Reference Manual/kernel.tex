...
\begin{definition}[Formulas]
  ...
  %
  \begin{align}
    \mathcal{L}_{Predicate} := & ~\ConstantPredKerLabel(\textnormal{Id}, \textnormal{Arity})    \\
...
\begin{example}[Formula]The following are typical examples of formula labels:
  \begin{align*}
    \True           & := \ConstantPredKerLabel(``\True", 0)             \\
    \False          & := \ConstantPredKerLabel(``\False", 0)            \\
    X               & := \SchematicPredicateLabel(``X", 0)                \\
    =               & := \ConstantPredKerLabel(``=", 2)                 \\
    {\in}           & := \ConstantPredKerLabel(``{\in}", 2)             \\
