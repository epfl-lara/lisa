 ...
 */
class CertifiedFastEquivalenceTest extends AnyFunSuite:
   ...
   *     certified symbol may cover several fast ones. Requiring only the forward map captures exactly this (and more
   ...
   *  free variables (sorted by original name), matching FastClausify's Skolem functions. Unlike `Clausal.Abstraction`
   ...
   *  ε-terms are absorbed into their enclosing symbol, exactly as FastClausify's opaque Skolem functions absorb
  ...
  test("certified and fast clausifier name AND Skolemize equivalently on 200 TPTP problems (fof + eq, seed 42)") {
    val tptpRoot = sys.env.get("TPTP").map(new File(_)).filter(_.isDirectory)
    assume(tptpRoot.isDefined, "set TPTP to the directory containing Problems/ to run this equivalence check")
    val root = tptpRoot.get
      ...
      if f.exists then
        ...
        parsedOpt.foreach { parsed =>
          ...
          for phi <- inputFormulas(parsed) if size(phi) <= 8000 do // skip giant formulas (findSite is superlinear)
            ...
            val fastSk = CertifiedFastClausifier.fastNamedNnfSkolem(phi) // linear (Skolem functions) — always cheap
            runWithTimeout(2000) { CertifiedFastClausifier.stripForall(absEps(CertifiedFastClausifier.namedNnfSkolemEps(phi))) } match
              case None => skolemTimeouts += 1; println(f"[timer] TIMEOUT  size=${size(phi)}%6d  $rel")
              case Some(certSk) =>
                if !isoUpToRenaming(fastSk, certSk) then skolemFails += ((rel, isoMismatch(fastSk, certSk)))
                formulasChecked += 1
