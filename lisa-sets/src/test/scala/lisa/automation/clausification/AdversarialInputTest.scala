 ...
 * load-bearing: several of these inputs trip more than one `require` on their way down (a malformed sequent
...
class AdversarialInputTest extends AnyFunSuite:
  ...
  private def sorryPhaseProver(p: Problem): ClausificationProof =
    ...
    ClausificationProof(sc.steps.map(KernelStep(_)), sc.imports ++ Clausification.libImports)
  ...
  // the most likely *external* fault — it is the one part of the pipeline a caller supplies.
  ...
  test("a well-behaved prover on the same problems is accepted — the checks above are not vacuous") {
  ...
  test("a genuine NNF matrix passes the same phase — the leaf check is not rejecting everything") {
  ...
  // premises. Getting that split wrong would mis-index every reference inside the lowered subproof, so it is
  // checked at construction rather than discovered during lowering.

  test("a subproof whose assumptions and premises do not account for every import is rejected") {
    val inner = ClausificationProof(IndexedSeq(KernelStep(Hypothesis(a |- a, a))), IndexedSeq(() |- a, () |- b))
    val e = intercept[IllegalArgumentException](ClausificationSubproof(inner, IndexedSeq.empty, IndexedSeq(Assumption(a, 0))))
  ...
  test("an assumption index outside the inner proof's imports is rejected") {
    val inner = ClausificationProof(IndexedSeq(KernelStep(Hypothesis(a |- a, a))), IndexedSeq(() |- a))
    val e = intercept[IllegalArgumentException](ClausificationSubproof(inner, IndexedSeq.empty, IndexedSeq(Assumption(a, 7))))
  ...
  test("two assumptions naming the same import are rejected") {
    val inner = ClausificationProof(IndexedSeq(KernelStep(Hypothesis(a |- a, a))), IndexedSeq(() |- a, () |- b))
    val e = intercept[IllegalArgumentException](
      ClausificationSubproof(inner, IndexedSeq.empty, IndexedSeq(Assumption(a, 0), Assumption(b, 0))))
