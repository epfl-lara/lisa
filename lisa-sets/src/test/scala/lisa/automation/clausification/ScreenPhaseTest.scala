 ...
 * [[Clausification.schemaP]] / [[Clausification.schemaR]] (Skolem and prenex bridges) and the fresh `esk…`/`nm…`
 ...
 * `InstSchema` under that assumption. Hoisting the renaming above the assumption — where the sequents still
 * have an empty left-hand side, so the substitution is legal at every sort — is what removes the whole class
 * rather than the instances, which is why the tests below are collisions of *kind* (a bridge schema, a prenex
 * operand, a generated Skolem symbol, a library binder) rather than a list of names.
 */
class ScreenPhaseTest extends AnyFunSuite:
  ...
  private val R   = Variable(Identifier("R"), Prop)             // == Clausification.schemaR (prenex laws)
  ...
  test("a goal over `R` — the closed operand of the prenex laws — clausifies to a kernel-valid proof") {
    // Negated + NNF: ∃w. (∀z.Q(z)) ∧ ¬Q(w) ∧ ¬R — the prenex phase lifts `∀z` over `∧` with the closed sibling
    // supplied as `R := …`, so a *free* R in the assumption is what used to break.
  ...
  test("a goal over `esk_1` — the first Skolem function SkolemPhase mints — clausifies to a kernel-valid proof") {
    // The negation `∀u.∃w.…` has a witness mentioning `u`, so SkolemPhase mints `esk_1 : Ind → Ind` — the user's
  ...
  test("an input binder that shadows a canonical target is α-renamed, and the goal is still restored verbatim") {
    ...
    // binder, so `σ⁻¹` gives back the goal only up to α — `certifyScreen` restores the caller's sequent itself.
  ...
  test("no η-reduced quantifier survives into the clauses handed downstream") {
    // The clause-level statement of the fix: before it, the prover was handed the literal `∀(usr)` — a
  ...
  test("after screening, every free variable of the problem is in the `v` / `P` / `F` namespaces") {
    ...
    assert(screened.forall(v => allowed.contains(v.id.name)), s"unscreened variables remain: ${screened.filterNot(v => allowed.contains(v.id.name))}")
    ...
    val schemas = Set(Clausification.schemaP, Clausification.schemaR, x)
