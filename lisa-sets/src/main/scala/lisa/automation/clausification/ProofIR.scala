...
private[clausification] final case class ClausificationSubproof(
  ...
  // the discharge loop, O(Q) times for the csub produced by certifyTseitinFlat).
...
// ─────────────────────────────────────────────────────────────────────────────
// Flat streaming proof writer.
//
// [[StreamSink]] emits kernel proof steps one at a time to a [[PrintWriter]]
// using a simple line-oriented format:
//
//   <global-idx>|<ClassName>|<comma-premises>|<sequent.repr>
//
// Top-level imports are referenced by negative indices (-1 = import 0, …).
// The caller is responsible for writing the import header and footer lines.
// ─────────────────────────────────────────────────────────────────────────────

private[clausification] class StreamSink(pw: java.io.PrintWriter) {
  private var _count = 0
  def emit(step: SCProofStep): Int = {
    val idx = _count
    pw.println(s"$idx|${step.getClass.getSimpleName}|${step.premises.mkString(",")}|${step.bot.repr}")
    _count += 1
    idx
  }
...
/** Inline a kernel [[SCProof]] into a [[StreamSink]], remapping all local
  * premise indices to global stream indices. Every [[SCSubproof]] is recursed
  * into (no inner [[SCProof]] is kept alive after its steps are written).
  * Returns the global index of the last emitted step (the proof's conclusion). */
private[clausification] def inlineKernelProofFlat(
    sp: SCProof,
    assumptions: Seq[Expression],
    outerPremGlobal: IndexedSeq[Int],
    sink: StreamSink
): Int = {
  val assumptionSet: Set[Expression] = if assumptions.isEmpty then Set.empty else assumptions.toSet
  val stepGlobalIdxs = scala.collection.mutable.ArrayBuffer.empty[Int]
  def mapRef(ref: Int): Int =
    if ref >= 0 then stepGlobalIdxs(ref)
    else outerPremGlobal(-ref - 1)
  sp.steps.foreach { step =>
    val globalRef = step match
      case SCSubproof(inner, innerPremises) =>
        inlineKernelProofFlat(inner, assumptions, innerPremises.map(mapRef).toIndexedSeq, sink)
      case _ =>
        val rebased = mapStepPremises(step, mapRef)
        val finalStep =
          if assumptionSet.isEmpty then rebased
          else rewriteStepBot(rebased, addAssumptionsLeftSet(rebased.bot, assumptionSet))
        sink.emit(finalStep)
    stepGlobalIdxs += globalRef
  }
  stepGlobalIdxs.last
}

/** Like [[lowerClausificationProof]] but writes each step to a [[StreamSink]]
  * immediately rather than accumulating them into an [[SCProof]].  Every
  * [[ClausificationSubproof]] is recursed into and its steps are inlined; no
  * intermediate [[SCProof]] objects are kept alive after their steps are written.
  *
  * @param importGlobalRefs  For each position `i` in `proof.imports`, the global
  *   stream index of the step that provides that sequent to this proof, or a
  *   negative value (`-k-1`) for a top-level import at position `k` of the outer
  *   file-level import table.  Positions corresponding to assumption imports are
  *   never read (they are materialised inside this call via [[Hypothesis]] steps).
  * @return the global stream index of the last emitted step (the proof's conclusion).
  */
private[clausification] def lowerClausificationProofFlat(
    proof: ClausificationProof,
    localAssumptions: IndexedSeq[Assumption],
    inheritedAssumptions: IndexedSeq[Expression],
    importGlobalRefs: IndexedSeq[Int],
    sink: StreamSink
): Int = {
  val assumptions    = inheritedAssumptions ++ localAssumptions.map(_.formula)
  val assumptionSet  = if assumptions.isEmpty then Set.empty[Expression] else assumptions.toSet
  ...
  val inheritedSet   = if inheritedAssumptions.isEmpty then Set.empty[Expression] else inheritedAssumptions.toSet

  // ── Build per-import global refs ──────────────────────────────────────────
  // extGlobalRefArr(i): global stream index (or negative top-level ref) for
  // import i after optional Weakening widening.
  val extGlobalRefArr = Array.fill(proof.imports.size)(Int.MinValue)
  externalImportIndices.foreach { oldIndex =>
    val incomingRef  = importGlobalRefs(oldIndex)
    val externalView = addAssumptionsLeftSet(proof.imports(oldIndex), inheritedSet)
    val newRef: Int =
      if localAssumptions.isEmpty then incomingRef
      else
        val target = addAssumptionsLeftSet(proof.imports(oldIndex), assumptionSet)
        if target.left.size == externalView.left.size && isSameSequent(target, externalView) then incomingRef
        else sink.emit(Weakening(target, incomingRef))
    extGlobalRefArr(oldIndex) = newRef
  }

  // ── Materialise local-assumption Hypothesis steps ─────────────────────────
  val localAssumptionGlobalRefs = scala.collection.mutable.HashMap.empty[Int, Int]
  localAssumptions.foreach { case Assumption(phi, importIndex) =>
    val hypRef     = sink.emit(Hypothesis(phi |- phi, phi))
    val weakBot    = addAssumptionsLeftSet(() |- phi, assumptionSet)
    val finalRef   =
      if weakBot.left.size == 1 && isSameSequent(weakBot, phi |- phi) then hypRef
      else sink.emit(Weakening(weakBot, hypRef))
    localAssumptionGlobalRefs(importIndex) = finalRef
  }

  // ── Per-step global-ref tracking ─────────────────────────────────────────
  val stepGlobalIdxs = scala.collection.mutable.ArrayBuffer.empty[Int]
  def mapRef(ref: Int): Int =
    if ref >= 0 then stepGlobalIdxs(ref)
    else
      val idx = -ref - 1
      val ext = extGlobalRefArr(idx)
      if ext != Int.MinValue then ext
      else localAssumptionGlobalRefs(idx)

  proof.steps.foreach {
    case KernelStep(step) =>
      if Thread.interrupted() then throw new InterruptedException("Clausification cancelled (flat lowering)")
      val globalRef = step match
        case SCSubproof(sp, premises) =>
          inlineKernelProofFlat(sp, assumptions, premises.map(mapRef).toIndexedSeq, sink)
        case _ =>
          val rebased   = mapStepPremises(step, mapRef)
          val finalStep =
            if assumptionSet.isEmpty then rebased
    ...
    case ClausificationSubproof(subproof, premises, subAssumptions) =>
      if Thread.interrupted() then throw new InterruptedException("Clausification cancelled (flat lowering)")
      val subAssumptionIndexSet     = subAssumptions.map(_.importIndex).toSet
      val subExternalImportIndices  = subproof.imports.indices.filterNot(subAssumptionIndexSet).toIndexedSeq
      val extIdxToPos               = subExternalImportIndices.zipWithIndex.toMap
      val subImportGlobalRefs       = IndexedSeq.tabulate(subproof.imports.size) { i =>
        extIdxToPos.get(i).map(j => mapRef(premises(j))).getOrElse(Int.MinValue)
      }
      val lastGlobal = lowerClausificationProofFlat(
        subproof, subAssumptions, assumptions, subImportGlobalRefs, sink
      )
      stepGlobalIdxs += lastGlobal
  }
