 ...
 * [[Clausal.solveOutcome]] and [[Strategy]]) takes and forwards the whole record rather than re-declaring a
 ...
 *                          active scan), applied once at creation. **Off by default**: 71 refuted vs 74, so it
 ...
 *                          demodulation with its upkeep), reducing the loop to ordered resolution and factoring.
