...
object BaselineBench:
  ...
  private def printRow(r: Row): Unit =
    ...
    println(Seq(r.name, r.category, ms(r.clausifyMs), ms(r.proverMs), ms(r.clausifyMs + r.proverMs), r.processed.toString, r.derived.toString).mkString("ROW\t", "\t", ""))
  ...
  private def run(dataset: String, n: Int, seed: Long, timeoutMs: Long, maxGiven: Int, maxSize: Int, precedence: PrecedenceScheme): Unit =
    ...
    println(
      s"# dataset=$dataset list=${lists(dataset)} seed=$seed n=${picked.size} timeout=${timeoutMs}ms maxGiven=$maxGiven maxSize=$maxSize equality=$eq precedence=$precedence (uncertified clausification, NO reconstruction)"
    )
    println("# ROW\tproblem\tresult\tclausify_ms\tprover_ms\ttotal_ms\tgiven\tderived")
    BenchUtil.resetAbandoned()
  ...
  private def measure(cprob: Problem, timeoutMs: Long, maxGiven: Int, equality: Boolean, precedence: PrecedenceScheme): Row =
    ...
    val outcome: Clausal.Outcome =
      try Clausal.solve(clausal, SearchOptions(equality = equality, precedenceScheme = precedence, maxGiven = maxGiven, maxMillis = timeoutMs, onStats = stats.set))
