...
object Strategy:
   ...
   */
  val weightGreedy = Strategy("weight-greedy", base.copy(weightRatio = 16, selection = BestLiteral, sine = Some(SineConfig(tolerance = 1.5, depth = 2))))
   ...
   */
  val ageFair = Strategy("age-fair", base.copy(ageRatio = 8, nonGoalWeightCoefficient = 1, sine = Some(SineConfig(tolerance = 5.0, depth = 0)), orthologic = true))
   ...
   */
  val occurrence = Strategy("occurrence", base.copy(weightRatio = 4, precedenceScheme = PrecedenceScheme.Occurrence, sine = Some(SineConfig(tolerance = 3.0, depth = 0)), orthologic = true))
   ...
   */
  val equational = Strategy("equational", base.copy(precedenceScheme = PrecedenceScheme.Arity, weightScheme = WeightScheme.Arity, sine = Some(SineConfig(tolerance = 3.0, depth = 3))))
   ...
   */
  val unaryRedundancy = Strategy(
    "unary-redundancy",
    base.copy(
      precedenceScheme = PrecedenceScheme.UnaryFirst,
      forwardSubsumptionResolution = true,
      backwardSubsumptionResolution = true,
      condensation = true,
      sine = Some(SineConfig(tolerance = 2.0, depth = 0)),
      orthologic = true
    )
  )
   ...
   */
  val subsumptionLight = Strategy(
    "subsumption-light",
    base.copy(
      nonGoalWeightCoefficient = 3,
      forwardSubsumptionResolution = true,
      backwardSubsumptionResolution = true,
      condensation = true,
   ...
   */
  val firstNegative = Strategy("first-negative", base.copy(selection = FirstNegative, nonGoalWeightCoefficient = 5, sine = Some(SineConfig(tolerance = 1.5, depth = 1)), orthologic = true))
