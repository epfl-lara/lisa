 ...
 * arithmetic-free TPTP **theorems** (`tptp-fof-fo-eq-thm.txt`) — the exact analog of the equality-free
 * [[FofEvaluation]] set, selected the very same way (by the TPTP `SPC` header) but with the equality
 * classes `FOF_THM_{RFO,EPR}_{SEQ,PEQ}` (**S**ome or **P**ure **EQ**uality) in place of `…_NEQ`. The `CSR`
 * (SUMO commonsense) domain is excluded for the same reason as there (giant numeric ontologies the parser
 * mishandles / that blow the size budget), and only `.p` problem files are kept (not TPTP's withdrawn `.rm`
 * variants), leaving 5589 clean equality-bearing FO theorems. Each sampled
 * problem is parsed, run through the certified clausifier ([[Clausification.certifyClausal]]) with
 * [[Clausal.prove]] as the back-end, and — on a refutation — the composed proof is kernel-checked. Paths are
 * relative to the TPTP root (e.g. `Problems/SEU/SEU140+2.p`).
 *
 * This is the set that actually exercises the Phase-4 equality inferences (superposition, equality
 * resolution/factoring, demodulation) end-to-end, so the `equality` arg is a meaningful ablation here
 * (unlike the equality-free [[FofEvaluation]], where it only measures the inference machinery's inert cost).
 *
 * Needs the `TPTP` env var pointing at the TPTP root (the directory containing `Problems/`). Run:
 * {{{
 *   TPTP=/path/to/TPTP-v9.2.1 sbt "lisa-sets/runMain lisa.automation.superposition.EqFofEvaluation [seed] [n] [timeoutMs] [maxGiven] [maxSize] [cert|uncert] [equality]"
 * }}}
 * The 6th arg `uncert` skips certification; the 7th arg `off` skips all equality inferences (superposition,
 * equality resolution/factoring, demodulation) — on this equality-bearing list `off` genuinely weakens the
 * prover (many refutations need equality reasoning), so it measures the equality inferences' contribution.
...
object EqFofEvaluation:
  ...
  /** The generated full equality-bearing FOF list file (paths relative to the TPTP root). */
  private val listFileName = "tptp-fof-fo-eq-thm.txt"

  /** Locate the FOF problem list: `TPTP_FOF_EQ_LIST`, else source-relative candidates / the cwd. */
  private def locateListFile(): Option[File] =
    val candidates: List[File] =
      sys.env.get("TPTP_FOF_EQ_LIST").map(new File(_)).toList :::
        List(
  ...
  def main(args: Array[String]): Unit =
    ...
    // `sample [n] [seed]` prints the (TPTP-root-relative) paths of the seeded draw and exits — so an external
    // prover (e.g. E) can be run on the exact same problem set for a head-to-head comparison.
    if args.headOption.contains("sample") then
      ...
      return
    // `files <listfile> [timeoutMs] [maxGiven] [cert|uncert] [maxSize]` runs an explicit list of TPTP-root-relative
    // problem paths (one per line) through the same pipeline as `benchmark`, with no sampling. `maxSize` defaults to
    // ∞ (the size guard is off) — for retrying the problems the default size cap skips.
    if args.headOption.contains("files") then
      runFiles(args.drop(1))
      return
    benchmark(
      ...
      fingerprintIndexing = args.lift(7).forall(_.toLowerCase != "noindex") // 8th arg "noindex" ⇒ linear-scan superposition
    )
  ...
  /** Run an explicit list of TPTP-root-relative problem paths (read from `a(0)`, one per line) through the
   *  same pipeline as [[benchmark]], with no sampling. Args: `<listfile> [timeoutMs=15000] [maxGiven=100000]
