...
private[functions] final class SemanticRecFunctionInternals[N <: Arity](
  functionName: String,
    adt: SemanticADT[N],
  ...
  def definitionFormula(v: Expr[Ind]) = untypedDefinition.substitute(f := v)
  ...
  // The witness is defined as a polymorphic class-level constant to avoid free-variable DEF errors.
  private val witnessClass: Constant[?] = {
    val witnessExpr: Expr[?] = lisa.utils.fol.FOL.Abs.apply(
      xs = typeVariablesSeq,
      t = { pairWitness ∈ (adt.term × returnType) | caseMembership(pairWitness) }
    )
    type S
  ...
  private val witnessDefinition = definitionFormula(witness)
  ...
  private val witnessCaseByConstructor: Map[SemanticConstructor[N], THM] =
  ...
  // TODO(recursion-principle): derive this from witness construction + typing.
  private val witnessDefinitionBridge: JUSTIFICATION = Lemma(witnessDefinition){
    ...
    // println(s"Witness definition bridge for $functionName:")
    // println(s"Witness definition: $witnessDefinition")
    // println(s"Witness cases: $witnessCases")

    // For the function length, we need to prove 
    // length/witness(A, list/cons[A](hd)(tl)) = nat/succ(length/witness(A, tl))
    // But the witness cases only give us
    // length/witness(A, list/cons[A](hd)(tl)) = nat/succ(length[A](tl))
    // We need to use the height to show that length/witness(A, tl) = length[A](tl) 
    // because h(tl) < h(list/cons[A](hd)(tl)).

    have(thesis) by Sorry
  }

  val uniqueness: THM = Lemma(existsOne(f, untypedDefinition)) {

    val constructorCaseFacts = cases.keys.toSeq.map(c => witnessCaseByConstructor(c))
    have(witnessCases) by Tautology.from(constructorCaseFacts*)

    have(witnessDefinition) by Restate.from(witnessDefinitionBridge)
