 ...
 */
   ...
   * classFunction represents the constructor as a set-theoretic function:
   *   classFunction * X1 * ... * Xm * x1 * ... * xn = (tagc, (x1, (..., (xn, ∅)...))
   * where Xi are type variables and xi are constructor arguments.
   * Formerly defined via:
   *   FunctionDefinition[N](fullName, ...)(typeVariablesSeq, c, untypedDefinition, uniqueness).label
   */
 ...
 */
  private val classFunctionCharacterization = Lemma(
    forall(c, (term === c) <=> untypedDefinition)
  ){
 ...
 */
  val shortDefinition = Lemma(
    using name=sourcecode.FullName(s"${fullName}/shortDefinition")
  )(forallSeq(variables, wellTypedFormula(semanticSignature) ==> (appliedTerm === structuralTerm))) {
    // have(forall(c, (term === c) <=> untypedDefinition)) by Exact(classFunction.definition)
    have(forall(c, (term === c) <=> untypedDefinition)) by Restate.from(classFunctionCharacterization)
 ...
 */
  val intro = 
    Lemma(using name=sourcecode.FullName(s"${fullName}/intro"))(forallSeq(typeVariablesSeq, term :: typ)) {
    // have(forall(c, (term === c) <=> untypedDefinition)) by Exact(classFunction.definition)
    have(forall(c, (term === c) <=> untypedDefinition)) by Restate.from(classFunctionCharacterization)
    thenHave((term === term) <=> ((term :: typ) /\ forallSeq(variables, wellTypedFormula(semanticSignature) ==> (appliedTerm === structuralTerm)))) by InstantiateForall(term)
 ...
 */
  lazy val injectivity = {
    ...
    if arity == 0 then {
      ...
      Lemma(using name=lemmaName)(vars1WellTyped ++ vars2WellTyped |- simplify((appliedTerm1 === appliedTerm2) <=> (variables1 === variables2))) {
        ...
        have((appliedTerm1 === appliedTerm2) <=> (structuralTerm1 === structuralTerm2) |- (appliedTerm1 === appliedTerm2) <=> seqEq(variables1,variables2)) by Sorry
        Cut(
          underlying.injectivity,
          equivalenceRewriting of (p1 := (appliedTerm1 === appliedTerm2), p2 := (structuralTerm1 === structuralTerm2), p3 := seqEq(variables1,variables2))
        )
