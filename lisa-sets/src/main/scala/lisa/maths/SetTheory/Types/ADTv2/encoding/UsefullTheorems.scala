...
object UsefullTheorems {
    ...
    /**
      ...
      * 
      * ADT/ is deprecated and should not be used
      * But the files are still here for reference
      */
    ...
    object QuantifiersIntro extends lisa.utils.prooflib.ProofTacticLib.ProofTactic {
         ...
         */
        def apply(using lib: lisa.utils.prooflib.Library, proof: lib.Proof)(vars: Seq[Variable[Ind]])(fact: proof.Fact)(bot: Sequent): proof.ProofTacticJudgement =
            TacticSubproof { sp ?=>
    ...
    val unionRangeCumulativeRestrictedFunction = Lemma(
        (functional(h), in(n, N), relationDomain(h) === N, forall(m, subset(m, n) ==> subset(app(h)(m), app(h)(n)))) |- unionRange(
