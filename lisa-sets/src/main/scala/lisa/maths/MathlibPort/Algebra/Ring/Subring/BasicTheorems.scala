 ...
 *
 * Lean source reference: `Mathlib/Algebra/Ring/Subring` (basic fragment).
 */
object BasicTheorems extends lisa.Main {
  ...
  val subring_intersection = Theorem(
    (Defs.subring(R)(add)(zero)(negOp)(mul)(one)(S), Defs.subring(R)(add)(zero)(negOp)(mul)(one)(T0)) |- Defs.subring(R)(add)(zero)(negOp)(mul)(one)(S ∩ T0)
  ) {
    assume(Defs.subring(R)(add)(zero)(negOp)(mul)(one)(S))
    thenHave((S ⊆ R) /\ (zero ∈ S) /\ (one ∈ S) /\ Defs.closedAdd(add)(S) /\ Defs.closedNeg(negOp)(S) /\ Defs.closedMul(mul)(S)) by Substitute(
      Defs.subring.definition of (
        Defs.R := R,
        Defs.add := add,
        Defs.zero := zero,
        Defs.negOp := negOp,
    ...
    assume(Defs.subring(R)(add)(zero)(negOp)(mul)(one)(T0))
    thenHave((T0 ⊆ R) /\ (zero ∈ T0) /\ (one ∈ T0) /\ Defs.closedAdd(add)(T0) /\ Defs.closedNeg(negOp)(T0) /\ Defs.closedMul(mul)(T0)) by Substitute(
      Defs.subring.definition of (
        Defs.R := R,
        Defs.add := add,
        Defs.zero := zero,
    ...
    val sClosedNeg = have(forall(x0, (x0 ∈ S) ==> (negApp(x0) ∈ S))) by Tautology.from(
      sData,
      Defs.closedNeg.definition of (Defs.negOp := negOp, Defs.S := S)
    )
    val tClosedNeg = have(forall(x0, (x0 ∈ T0) ==> (negApp(x0) ∈ T0))) by Tautology.from(
      tData,
      Defs.closedNeg.definition of (Defs.negOp := negOp, Defs.S := T0)
    )
        ...
        have((x0 ∈ (S ∩ T0)) ==> forall(y0, (y0 ∈ (S ∩ T0)) ==> (addApp(x0, y0) ∈ (S ∩ T0)))) subproof {
          ...
          have(forall(y0, (y0 ∈ (S ∩ T0)) ==> (addApp(x0, y0) ∈ (S ∩ T0)))) subproof {
            have((y0 ∈ (S ∩ T0)) ==> (addApp(x0, y0) ∈ (S ∩ T0))) subproof {
              ...
              thenHave(thesis) by RightImplies.withParameters(y0 ∈ (S ∩ T0), addApp(x0, y0) ∈ (S ∩ T0))
            }
    ...
    have(Defs.closedNeg(negOp)(S ∩ T0)) subproof {
      have(forall(x0, (x0 ∈ (S ∩ T0)) ==> (negApp(x0) ∈ (S ∩ T0)))) subproof {
        have((x0 ∈ (S ∩ T0)) ==> (negApp(x0) ∈ (S ∩ T0))) subproof {
          val xInST = assume(x0 ∈ (S ∩ T0))
          val xInBoth = have((x0 ∈ S) /\ (x0 ∈ T0)) by Tautology.from(mem_intersection_iff of (x0 := x0, S := S, T0 := T0), xInST)
          val xInS = have(x0 ∈ S) by Tautology.from(xInBoth)
          val xInT = have(x0 ∈ T0) by Tautology.from(xInBoth)

          val negInS = have(negApp(x0) ∈ S) by Tautology.from(sClosedNeg of x0, xInS)
          val negInT = have(negApp(x0) ∈ T0) by Tautology.from(tClosedNeg of x0, xInT)
          have(negApp(x0) ∈ (S ∩ T0)) by Tautology.from(
            mem_intersection_iff of (x0 := negApp(x0), S := S, T0 := T0),
            negInS,
            negInT
          )

          thenHave(thesis) by RightImplies.withParameters(x0 ∈ (S ∩ T0), negApp(x0) ∈ (S ∩ T0))
        }

        thenHave(thesis) by RightForall.withParameters((x0 ∈ (S ∩ T0)) ==> (negApp(x0) ∈ (S ∩ T0)), x0)
      }

      thenHave(thesis) by Substitute(Defs.closedNeg.definition of (Defs.negOp := negOp, Defs.S := (S ∩ T0)))
    }
    val stClosedNeg = lastStep
        ...
        have((x0 ∈ (S ∩ T0)) ==> forall(y0, (y0 ∈ (S ∩ T0)) ==> (mulApp(x0, y0) ∈ (S ∩ T0)))) subproof {
          ...
          have(forall(y0, (y0 ∈ (S ∩ T0)) ==> (mulApp(x0, y0) ∈ (S ∩ T0)))) subproof {
            have((y0 ∈ (S ∩ T0)) ==> (mulApp(x0, y0) ∈ (S ∩ T0))) subproof {
              ...
              thenHave(thesis) by RightImplies.withParameters(y0 ∈ (S ∩ T0), mulApp(x0, y0) ∈ (S ∩ T0))
            }
