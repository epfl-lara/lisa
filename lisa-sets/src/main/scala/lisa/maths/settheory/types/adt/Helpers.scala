...
object QuantifiersIntro extends lisa.prooflib.ProofTacticLib.ProofTactic {
  ...
  def apply(using lib: lisa.prooflib.Library, proof: lib.Proof)(vars: Seq[Variable], rightToLeft: Boolean = true)(fact: proof.Fact)(bot: Sequent): proof.ProofTacticJudgement =
    TacticSubproof { sp ?=>
...
private [adt] object Helpers {
  ...
  /**
   * Extracts the variables from a term
   */
  def variables(t: Term): Seq[Variable] =
    t match
      case v: Variable => Seq(v)
      case _: Constant => Seq.empty
      case af: AppliedFunctional => af.args.flatMap(variables)
...
private[adt] object ADTDefinitions {
  ...
  /**
   * Extracts the type variables from a constructor argument
   */
  def variables(arg: ConstructorArgument): Seq[Variable] = arg match
    case Self => Seq.empty
    case GroundType(t) => Helpers.variables(t)
